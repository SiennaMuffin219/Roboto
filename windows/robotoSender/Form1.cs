...
namespace AlphaROBOTO
{
    public partial class Form1 : Form
    {
        ...
        private void Form1_MouseMove(object sender, MouseEventArgs e)
        {
            ...
            if (IsMouseDown && x > 0 && x < 200 && y > 0 && y < 200)
            {
                ...
                if (x >= 100 && x <= 120)
                {
                    ...
                    Send(SpeedL, SpeedR).GetAwaiter().GetResult();
                }
                else
                {
                    if (x < 100)
        ...
        private async Task Send(int SpeedL, int SpeedR)
        {
            
            await client.DownloadStringTaskAsync($"http://roboto.elyspioweb.xyz/assets/php/android.php?speedL={SpeedL}&speedR={SpeedR}");
        }
    }
}
